...
import { outcomeLabel, throwJiaobei, type JiaobeiFace, type JiaobeiOutcome } from "./domain/jiaobei";
...
export default function App() {
    ...
    const runNumberReveal = useCallback(
        (target: number, durationMs: number): void => {
            const tickSoundId = window.setInterval(() => audio.playRollTick(soundOn), 95);
    ...
    const runWheelReveal = useCallback(
        (target: number, durationMs: number, meta: Readonly<{ n: number; idx: number }>): void => {
            if (meta.idx < 0) {
                setIsRolling(false);
                return;
            }

            const tickSoundId = window.setInterval(() => audio.playRollTick(soundOn), 110);
            tickTimerRef.current = tickSoundId;
    ...
    const performThrow = useCallback(() => {
        if (isTossing) return;
        setIsTossing(true);
        const tickSoundId = window.setInterval(() => audio.playJiaobeiTick(soundOn), 105);
        tickTimerRef.current = tickSoundId;

        const shuffleFaces = window.setInterval(() => {
        ...
        window.setTimeout(() => {
            stopTickTimers();
            window.clearInterval(shuffleFaces);
            const finalThrow = throwJiaobei();
            setJiaobeiLeft(finalThrow.left);
            setJiaobeiRight(finalThrow.right);
            setJiaobeiResult(finalThrow.outcome);
            setJiaobeiHistory((h) => prependCapped(h, finalThrow, HISTORY_CAP));
    ...
    const clearJiaobeiHistory = (): void => setJiaobeiHistory([]);
    ...
    const tossResultText = jiaobeiResult === null ? "?" : outcomeLabel(jiaobeiResult);

    const wheelSlices = useMemo(() => (wheelOk ? buildWheelSlices(range) : []), [range, wheelOk]);
        ...
        <div className="app">
            <div className="card">
                <header className="header">
                    ...
                    <span className={`status${isRolling || isTossing ? " rolling" : ""}`}>
                ...
                {page === "number" ? (
                    <>
                        ...
                        {!bounds.ok && <p className="err">請輸入有效的數字範圍。</p>}
                        ...
                        <div className="options">
                            ...
                            <button type="button" className="reset-pool" onClick={resetPool} disabled={!noRepeat}>
                                重置抽籤池
                            </button>
                        </div>

                        <section className="result-shell" aria-live="polite">
                            <div className="result-label">{mode === "wheel" && wheelOk ? "WHEEL" : "RESULT"}</div>
                                    ...
                                    <div className="wheel-stack">
                                        <div className="wheel-wrap">
                                            ...
                                            <div className="wheel-disk wheel-disk--svg">
                                                <svg className="wheel-svg" viewBox="-50 -50 100 100" aria-hidden>
                                                    <g
                                            ...
                                            <div className="wheel-center">{display === null ? "?" : display}</div>
                                        </div>
                                        <p className="wheel-caption">可視轉盤 {rangeCount} 格</p>
                                    </div>
                                ) : (
                            ...
                            <p className="result-sub">
                                {poolEmpty ? "抽籤池已空，請重置或關閉不重複" : isRolling ? "Rolling…" : "Press Roll"}
                            </p>
                        </section>
                        ...
                        <div className="actions">
                            ...
                            <button type="button" className="btn-secondary" onClick={() => setSoundOn((s) => !s)}>
                        ...
                        <footer className="footer">
                            ...
                            <span className="footer-center">Range {bounds.ok ? `${lo}–${hi}` : "—"}</span>
                    ...
                    <>
                        <section className="result-shell" aria-live="polite">
                            <div className="result-label">JIAOBEI</div>
                            <div className="result-main">
                            ...
                            <p className="result-sub">{isTossing ? "擲筊中…" : tossResultText}</p>
                        </section>
                        ...
                        <div className="history-head">
                            <span className="history-title">JIAOBEI HISTORY</span>
                        </div>
                        <div className="history-actions single">
                            <button type="button" className="btn-secondary" onClick={clearJiaobeiHistory}>
                                清空紀錄
                            </button>
                        </div>
                        <div className="history-list">
                            {jiaobeiHistory.length === 0 ? (
                                ...
                                jiaobeiHistory.map((item, i) => (
                                    <div key={`${item.outcome}-${i}`} className="history-item">
                                        <span>#{jiaobeiHistory.length - i}</span>
                                        <span className="history-val">{outcomeLabel(item.outcome)}</span>
                                    </div>
                                ))
                            )}
                        </div>
                        ...
                        <footer className="footer">
                            <span>Enter / Space</span>
                            <span className="footer-center">聖筊 / 笑筊 / 陰筊</span>
                            <span className="footer-right">Designed by Gomez</span>
                        </footer>
                    </>
                )}
